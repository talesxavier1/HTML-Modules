...
export class Diagram {
    ...
    private onRequestEditOperation = (event: any) => {
        ...
        /* Shapes que nao pode ter conexao*/
        let shapes_c3d5bb54 = ["processContainer", "exceptionSubprocess"]
        ...
        pipelineArray.push(valid_c3d5bb54);
        ...
        const valid_a36f7f08 = (event: any) => {
            ...
            let fromKey = connector?.fromKey;
            if (!fromKey) { return true; }

            let fromShape = event.component.getItemById(fromKey);
        ...
        pipelineArray.push(valid_a36f7f08);
        ...
        const valid_925ba7a5 = (event: any) => {
            ...
            let shapeType = event?.args?.shape?.type ?? "";
            if (!shapes_925ba7a5.includes(shapeType)) { return true }

            let containerID = event?.args?.shape?.containerId;
            if (!containerID) { return false; }

            let container = event.component.getItemById(containerID);
            if (!container || container.type != "exceptionSubprocess") { return false; }
        ...
        pipelineArray.push(valid_8ee49461);

        /* Shapes que não podem ser incluidos em containers */
        let shapes_32fc4e41 = ["sender", "processContainer", "reciver"];
        const valid_3830272b = (event: any) => {
            if (!["addShape", "moveShape"].includes(event.operation)) { return true }

            let shapeType = event?.args?.shape?.type ?? "";
            if (!shapes_32fc4e41.includes(shapeType)) { return true }
        ...
        const valid_50e77487 = (event: any) => {
            ...
            if (!fromKey) { return true }
            let fromShape = event.component.getItemByKey(fromKey);
            if (!fromShape || fromShape.type != "startProcess") { return true }

            let toKey = event?.args?.connector?.toKey;
        ...
        const valid_62b9b95d = (event: any) => {
            ...
            let toShape = event.component.getItemByKey(toKey);
            if (toShape.type != "startProcess") { return true }

            let fromShape = event.component.getItemByKey(fromKey);
            if (!shapes_62b9b95d.includes(fromShape.type)) { return false }
...
class SenderCustomShape {
    ...
    private _shape: CustomShape.TCustomShape = {
        type: "sender",
        title: "Sender",
        category: "Process",
        defaultHeight: 2,
    ...
    get shape(): CustomShape.TCustomShape {
        return this._shape;
    }
}
